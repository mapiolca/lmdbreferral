...
$langs->loadLangs(array('users', 'companies', 'propal', 'lmdbreferral@lmdbreferral'));
...
if (!$object->id || !isModEnabled('lmdbreferral')) {
	accessforbidden();
}
if ($object->id != $user->id && !lmdbreferralCanDo($user, 'read')) {
	accessforbidden();
}
if ($object->id == $user->id && !lmdbreferralCanDo($user, 'own') && !lmdbreferralCanDo($user, 'read')) {
	accessforbidden();
}

llxHeader('', $langs->trans('LmdbReferralTabReferrals'));
$head = user_prepare_head($object);
print dol_get_fiche_head($head, 'lmdbreferral_filleuls', $langs->trans('User'), -1, 'user');
$linkback = '';
if ($user->hasRight('user', 'user', 'read') || $user->admin) {
	$linkback = '<a href="'.DOL_URL_ROOT.'/user/list.php?restore_lastsearch_values=1">'.$langs->trans('BackToList').'</a>';
}
dol_banner_tab($object, 'id', $linkback, 1, 'rowid', 'login');
print '<div class="underbanner clearboth"></div>';
lmdbreferral_print_user_filleuls_table($db, $langs, (int) $object->id);
print dol_get_fiche_end();
llxFooter();
$db->close();

/**
 * Print user filleuls table.
 *
 ...
 * @return void
 */
function lmdbreferral_print_user_filleuls_table($db, $langs, $userId)
{
	global $conf;
	...
	$where = array(
		...
		"l.referrer_type = 'user'",
		'l.fk_user_parrain = '.((int) $userId),
	);
	if ($searchStatus > 0) {
